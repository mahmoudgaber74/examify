...
`);
const adminClient = client(admin); const a = client(studentA); const b = client(studentB);
const started = await check('Student A starts Attempt A through RPC', a.rpc('start_exam_attempt', { p_exam_id: ids.exam }), true);
...
const duplicate = await check('same in_progress attempt is idempotently restored', a.rpc('start_exam_attempt', { p_exam_id: ids.exam }), true);
record('idempotent attempt identity', attemptId, duplicate.data?.id, duplicate.data?.id === attemptId);
...
await check('student cannot read unpublished graded result', a.from('exam_attempts').select('status,score').eq('id', attemptId).single(), false);
const finalAttempt = await check('admin reloads submitted attempt', adminClient.from('exam_attempts').select('status,score').eq('id', attemptId).single(), true);
record('final attempt status', 'graded', finalAttempt.data?.status, finalAttempt.data?.status === 'graded');
