-- Phase B.2: every finalized v2 page carries independently decodable identity.
...
BEGIN
  ...
  IF v_sheet.include_student_name THEN v_header_y := v_header_y + 8; END IF;
  IF v_sheet.include_student_id THEN v_header_y := v_header_y + 8; END IF;
  v_header_y := v_header_y + 4;
  ...
  RETURN jsonb_build_object(
    ...
    'page_identity_region', CASE WHEN COALESCE(v_sheet.layout_schema_version, 1) = 2 AND v_sheet.include_qr THEN jsonb_build_object(
      'x', (v_page_width - 15 - 25) / v_page_width,
      'y', (v_header_y - 5) / v_page_height,
      'width', 25 / v_page_width,
      'height', 25 / v_page_height
    ) ELSE NULL END,
    'sections', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'snapshot_section_id', s.id, 'section_key', s.section_key, 'title', s.title,
        'visual_index', s.visual_index, 'question_start_index', s.question_start_index,
        'question_count', s.question_count, 'page_number', s.page_number,
        'rect', jsonb_build_object('x', s.normalized_x, 'y', s.normalized_y,
          'width', s.normalized_width, 'height', s.normalized_height)
      ) ORDER BY s.visual_index)
      FROM public.bubble_sheet_sections s WHERE s.bubble_sheet_id = v_sheet.id
    ), '[]'::jsonb),
    'questions', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'snapshot_question_id', q.id, 'exam_question_id', q.exam_question_id,
        'question_id', q.question_id, 'question_type', q.question_type,
        'points', q.points_snapshot, 'question_ordinal', q.question_ordinal,
        'global_question_number', q.global_question_number,
        'section_visual_index', s.visual_index, 'section_question_number', q.section_question_number,
        'page_number', q.page_number,
        'rect', jsonb_build_object('x', q.normalized_x, 'y', q.normalized_y,
          'width', q.normalized_width, 'height', q.normalized_height),
        'options', COALESCE((
          SELECT jsonb_agg(jsonb_build_object(
            'snapshot_option_id', o.id, 'option_id', o.option_id, 'label', o.option_label,
            'canonical_option_ordinal', o.canonical_option_ordinal, 'visual_index', o.visual_index,
            'rect', jsonb_build_object('x', o.normalized_x, 'y', o.normalized_y,
              'width', o.normalized_width, 'height', o.normalized_height)
          ) ORDER BY o.visual_index)
          FROM public.bubble_sheet_options o WHERE o.bubble_sheet_question_id = q.id
        ), '[]'::jsonb)
