...
export function Assessment() {
  ...
  const [tab, setTab] = useState<'overview' | 'exams' | 'bank'>('overview');
  ...
  return <div className="space-y-6">
    ...
    <div className="flex gap-1 p-1 bg-white rounded-xl border border-ink-100 w-fit">{[['overview', 'نظرة عامة'], ['exams', 'الامتحانات'], ['bank', 'بنك الأسئلة']].map(([value, label]) => <button key={value} onClick={() => setTab(value as typeof tab)} className={`px-4 py-2 rounded-lg text-sm font-600 ${tab === value ? 'bg-brand-600 text-white' : 'text-ink-600 hover:bg-ink-50'}`}>{label}</button>)}</div>
