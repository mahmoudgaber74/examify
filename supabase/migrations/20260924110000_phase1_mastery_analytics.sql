-- Phase 1D: server-side mastery analytics.
-- A question linked to multiple outcomes contributes its points according to
-- the stored link weight; its full score is never duplicated per outcome.

CREATE INDEX IF NOT EXISTS exam_attempts_analytics_scope_idx
  ON public.exam_attempts (exam_id, status, is_result_published, student_id);

CREATE INDEX IF NOT EXISTS answers_analytics_scope_idx
  ON public.answers (attempt_id, question_id, is_correct);
...
BEGIN
  ...
  outcome_questions AS (
    SELECT qlo.learning_outcome_id, eq.question_id, eq.points,
      qlo.weight, lo.code, lo.name_ar, lo.subject_id
  ...
  SELECT g.learning_outcome_id, g.code, g.name_ar, g.subject_id,
    ...
    CASE WHEN g.possible_points = 0 THEN NULL::numeric
      ELSE round(100 * g.earned_points / g.possible_points, 2) END AS mastery_percentage,
    CASE
      WHEN g.attempt_count < 5 OR g.possible_points = 0 THEN 'insufficient_data'
      WHEN 100 * g.earned_points / g.possible_points < 50 THEN 'needs_review'
      WHEN 100 * g.earned_points / g.possible_points < 70 THEN 'developing'
      ELSE 'good'
    END AS classification,
    CASE
      WHEN g.attempt_count < 5 OR g.possible_points = 0 THEN 'بيانات غير كافية للحكم'
      WHEN 100 * g.earned_points / g.possible_points < 50 THEN 'يحتاج مراجعة أساسية'
      WHEN 100 * g.earned_points / g.possible_points < 70 THEN 'يحتاج تدريبًا إضافيًا'
      ELSE 'مستوى إتقان جيد'
    END AS recommendation,
    (g.attempt_count >= 5 AND g.possible_points > 0) AS is_sufficient
  FROM grouped g
  ORDER BY g.code, g.learning_outcome_id;
END;
$$;
