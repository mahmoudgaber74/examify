...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(saved_option) ORDER BY saved_option.sort_order), '[]'::jsonb)
  INTO v_saved_options
  FROM public.question_options saved_option
  WHERE saved_option.question_id = v_question_id;
