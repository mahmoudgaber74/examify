...
declare
  ...
  v_summary text := 'تم إنشاء تقدير داخلي يحتاج مراجعة بشرية.';
...
begin
  ...
  if length(trim(v_answer)) = 0 then
    ...
    return jsonb_build_object(
      ...
      'summary', 'الإجابة فارغة وتحتاج مراجعة.',
  ...
  if p_question_type = 'short_answer' then
    if v_normalized_model <> '' and v_normalized_answer = v_normalized_model then
      ...
      v_confidence := 0.95;
      v_summary := 'إجابة مطابقة للإجابة النموذجية.';
      ...
      v_confidence := 0.78;
      v_summary := 'إجابة قريبة من الإجابة النموذجية.';
    else
      ...
      v_summary := 'تم تقدير الإجابة القصيرة وفق التشابه مع الإجابة النموذجية.';
    end if;
    ...
    v_summary := 'تم تقدير الإجابة المقالية وفق rubrics الطول والمحتوى.';
  end if;
