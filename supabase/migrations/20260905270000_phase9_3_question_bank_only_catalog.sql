/* Only mapped Question Bank products are actionable in the current Marketplace. */
...
BEGIN
  ...
  IF product.type <> 'question_bank' THEN RAISE EXCEPTION 'product_fulfillment_unavailable'; END IF;
  ...
  RETURN result;
END; $$;
REVOKE ALL ON FUNCTION public.add_marketplace_item_to_cart(text) FROM PUBLIC, anon, authenticated;
