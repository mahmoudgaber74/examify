/* Restore the parent notification side effect lost in a later trigger rewrite. */
BEGIN;
...
BEGIN
  ...
  IF NEW.is_result_published IS NOT TRUE OR NEW.score IS NULL THEN
    ...
    DELETE FROM public.parent_notifications WHERE type = 'grade_posted' AND data->>'attempt_id' = NEW.id::text;
    RETURN NEW;
  END IF;
  ...
  VALUES (exam_row.institution_id, NEW.student_id, exam_row.subject_id, NEW.exam_id, NEW.id, exam_row.title, NEW.score, exam_row.total_points, COALESCE(NEW.approved_at, NEW.graded_at, NEW.submitted_at, now()));
  ...
  LOOP
    notification_key := 'grade_posted:' || NEW.id::text || ':' || linked_parent.parent_id::text;
    INSERT INTO public.parent_notifications (institution_id, parent_id, student_id, type, title, body, data, dedupe_key)
    VALUES (
      exam_row.institution_id, linked_parent.parent_id, NEW.student_id, 'grade_posted',
      'Exam result published', exam_row.title,
